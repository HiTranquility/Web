...
/**
 * Starts an embedded Tomcat 9 so the chapter 2 application can be run with a
...
public class DevServer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String contextPath = System.getProperty("contextPath", "/ch02email");
        ...
        tomcat.start();
        System.out.println();
        System.out.println("  Email List app running at http://localhost:"
                + port + contextPath + "/");
        System.out.println("  Data store: " + murachDataStore());
        System.out.println("  Press Ctrl+C to stop.");
        System.out.println();
        tomcat.getServer().await();
    }

    private static String murachDataStore() {
        String configured = System.getProperty("murach.emaillist.file");
        if (configured != null && !configured.isEmpty()) {
            return configured;
        }
        return System.getProperty("user.home") + File.separator + "murach"
                + File.separator + "EmailList.txt";
    }
}
